...
public class LinkedListItr implements ListItr {
     ...
     * Construct the list.
     *
     * @param anyList a LinkedList object to which this iterator is permanently
     * bound. This constructor is provided for convenience. If anyList is not a
     * LinkedList object, a ClassCastException will result.
     */
...
*/
    /**
     * Construct the list. As a result of the construction, the current position
     * is the first item, unless the list is empty, in which case the current
     * position is the zeroth item.
     *
     * @param anyList a LinkedList object to which this iterator is permanently
     * bound.
     */
    ...
    /**
     * Set the current position to the first node containing an item. current is
     * unchanged if x is not found.
     *
    ...
    /**
     * Remove the first occurrence of an item. current is set to the first node
     * on success; remains unchanged otherwise.
     *
     ...
     *
     * @return true if the current position is not null and is not referencing
     * the header node.
     */
    ...
    /**
     * Set the current position to the first node in the list. This operation is
     ...
     * position is null, then do nothing. No exceptions are thrown by this
     * routine because in the most common use (inside a for loop), this would
     * require the programmer to add an unnecessary try/catch block.
     */
    public void advance() {
        if (current != null) {
